...
const SimpleLocationInput = ({
  ...
  const [latitude, setLatitude] = useState(
    initialPosition[0]?.toString() || ""
  );
  const [longitude, setLongitude] = useState(
    initialPosition[1]?.toString() || ""
  );
  ...
  return (
    <div className="w-full rounded-xl overflow-hidden border-2 border-gray-200 shadow-lg">
      <div
        className="relative bg-gray-50 p-6"
      ...
      >
        ...
        <p className="text-sm text-gray-600 mb-6">
          Enter latitude and longitude coordinates manually. You can find
          coordinates using{" "}
          <a
        ...
        <div className="mt-6 p-4 bg-yellow-50 rounded-lg border border-yellow-200">
          <p className="text-xs text-yellow-800">
            <strong>Tip:</strong> Right-click on any location in Google Maps and
            select "What's here?" to see coordinates.
          </p>
        </div>
      </div>
    </div>
